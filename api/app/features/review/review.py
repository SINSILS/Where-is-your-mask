...
__DEFAULT_MASK_POINTS = [
    CoordinateIndex(120, 345, 93),
    CoordinateIndex(275, 385, 197),
    CoordinateIndex(423, 345, 323),
    CoordinateIndex(423, 245, 288),
    CoordinateIndex(305, 170, 400),
    CoordinateIndex(245, 173, 140),
...
def __try_create_review_image(raw_face_image: bytes, mask: MaskImage):
    # # Visualize the points
    # for i in range(0, np.size(mask_points)):
    #     x = mask_points[i].x
    #     y = mask_points[i].y
    ...
    cv2.add(image_head_noface, image_new_face)
...
def preprocess_mask_image(raw_mask_image: bytes):
    mask_img = __raw_image_to_cv2(raw_mask_image)
...
def create_review_image(raw_face_image: bytes, mask: MaskImage):
    try:
        return __try_create_review_image(raw_face_image, mask)
    except Exception:
